...
if __name__ == '__main__':
    ...
    p.add_option('-l', '--acc_len', dest='acc_len', type='int',default=5*(2**28)/1024,
        help='Set the number of vectors to accumulate between dumps. default is (2^28)/1024.')
    p.add_option('-g', '--gain', dest='gain', type='int',default=160,
    ...
    if opts.boffile != '':
        ...
        boffile = 'correlator_2017_Mar_22_2340.bof'
        #boffile = 'correlator_dos_adc_2017_Jan_25_1230.bof'
...
try:
    ...
    if not opts.skip:
        ...
        print 'Skipped.'
    #fpga.progdev('correlator_dos_adc_2017_Jan_19_1611.bof')
    #print 'done'
    ...
    fpga.write_int('quant_gain',opts.gain) #write the same gain for all inputs, all channels
    ...
    for chan in range(1024):
        #print '%i...'%chan,
        sys.stdout.flush()
        fpga.blindwrite('quant_addr',struct.pack('>I',chan))
    print 'done'

    print "ok, all set up. Try using poco_adc_amplitudes.py or poco_plot_adc.py to determine the " + \
          "adc input level, then try plotting using correlator_plot_auto.py or correlator_plot_cross.py"
